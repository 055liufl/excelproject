...
| G-07 | 单写者唯一 | `tst_sync_context_single_writer` | 同一时刻仅一个写事务持锁，并发写被串行化/拒绝 |
...
>
> **⚠️ qmake 前置任务（针对同步测试）**：当前 `src/libdbridge.pro` 的 `SOURCES` 仅列出 ETL 相关文件，**不包含** `src/sync/**` 与 `src/batch/BatchTransfer.cpp`。在新增同步测试套件并接入 qmake 双轨前，需先同步 `src/libdbridge.pro` 与根 `CMakeLists.txt` 的 `DBRIDGE_SOURCES`（对齐 `src/sync/**/*.cpp` 与 `src/batch/BatchTransfer.cpp`），否则 qmake 路径链接时会出现未解析符号。CI 中新增「CMake/qmake 源文件清单一致性校验」步骤：`diff <(grep -E '\.cpp' src/libdbridge.pro | sort) <(grep -E 'src/sync|src/batch' CMakeLists.txt | grep '\.cpp' | sort)`，不一致则 CI 失败。
...
参考流水线步骤（**统一使用 `DBRIDGE_COVERAGE=ON` + `CMAKE_BUILD_TYPE=Debug`**，消除前后命令不一致问题）：

```bash
# 配置（开启测试与覆盖率）——使用 Debug + DBRIDGE_COVERAGE，不使用 CMAKE_BUILD_TYPE=Coverage
cmake -S . -B build-cov \
  -DBUILD_TESTING=ON \
  -DDBRIDGE_COVERAGE=ON \
...
# 覆盖率采集（gcov 数据已在 ctest 运行后产出于 build-cov/）
gcovr -r . --object-directory build-cov \
  --exclude '3rdparty' --exclude 'tests' --exclude 'examples' \
  --exclude '.*moc_.*' --exclude '.*build.*' --exclude '.*/usr/.*' \
  --fail-under-line 80 \
  --fail-under-branch 70 \
  --xml-pretty -o coverage.xml --html-details coverage.html

# qmake 源文件清单一致性校验（见 §2.2.2 前置任务）
diff <(grep -oE 'src/[^ ]+\.cpp' src/libdbridge.pro | sort) \
     <(grep -oE 'src/[^ ]+\.cpp' CMakeLists.txt | sort) \
  || { echo "FAIL: libdbridge.pro and CMakeLists.txt SOURCES diverged"; exit 1; }
```

> `--fail-under-line 80 --fail-under-branch 70` 同时设置行/分支闸门；产出 `coverage.xml`（CI 插件）与 `coverage.html`（归档）。**注意**：不要使用 `-DCMAKE_BUILD_TYPE=Coverage`，该模式在 CMake 中不是内置类型，不等价于 `DBRIDGE_COVERAGE=ON`。
...
| `02_composite_pk.sql` | 复合主键表（`PRIMARY KEY(a,b)`） | **MVP 不支持复合 PK → 用于断言 `E_SYNC_COMPOSITE_PK_NOT_SUPPORTED` 拒绝路径**；亦供 FkClosureBuilder 解析复合 PK 父表引用测试 |
...
**职责**：构造两个独立节点 A（center）与 B（edge），各自拥有**独立库 + `wconn` + 独立 outbox/inbox 目录**（A.outbox ≠ B.inbox，B.outbox ≠ A.inbox），用于端到端收敛测试。目录独立是设计关键：`deliver()` 负责原子搬运，模拟第三方传输黑盒。可扩展为三/四节点用于星型拓扑。
...
- **`deliver()`**：模拟第三方传输——把 A.outbox 中已 `.ready` 的制品复制到 B.inbox，把 B.outbox 中已 `.ready` 的制品复制到 A.inbox。搬运顺序：先复制 `.payload` 到目标目录，再复制 `.ready` 哨兵，保证接收端先见哨兵时主文件已完整（FR-4 原子可见性）。A/B outbox 和 inbox 目录均独立，互不重叠。
...
**断言对象（五结构原子集）**：业务行、`__sync_changelog`、`__sync_applied_vector`、`__sync_row_winner`、`__sync_inbox_ledger`。注意 `inboxLedgerConsumed` 不参与事务提交，断言需按注入点拆分：

| 注入点 | 业务行/changelog/applied_vector/row_winner（事务原子集 4 项） | inboxLedger.status |
|---|---|---|
| `before_commit` | **全未发生** | `Seen`（未到 consumed） |
| `after_seal` | **全未发生**（seal 与业务同事务，COMMIT 前仍回滚） | `Seen` |
| `after_commit` | **全已发生**（COMMIT 已原子落盘） | `Seen`（consumed 标记在 COMMIT 之后、为独立操作）；重启后通过 applied_vector 幂等跳过，最终标 `consumed` |
| `after_markconsumed` | **全已发生** | `Consumed` |

`isAtomic()` 按上述分组校验：`before_commit`/`after_seal` 断言 4 项全 false；`after_commit` 断言 4 项全 true 且 ledger=Seen（非 Consumed）；`after_markconsumed` 断言 5 项全 true。
...
**可控时钟**：被测时间相关组件（`AckChannel`、`InboxLedger::stalePending`、`OutboundAckStore`、`SyncWorker` 主循环 `ackDeadlineMs_`、`DeadPeerEvictor` 阈值）当前直接调用 `QDateTime::currentMSecsSinceEpoch()`，这是测试抖动的根本来源。

**对被测实现的测试性要求**：各组件须支持 `nowMs` 可注入。推荐实现方式：在 `SyncConfig::Builder` 增加 `setNowFn(std::function<qint64()>)` 字段（测试构建时传 `FakeClock::fn()`，生产默认留空 → 回退 `QDateTime::currentMSecsSinceEpoch()`），并在以下点替换：

| 组件 | 当前用法 | 注入替换 |
|---|---|---|
| `AckChannel::flush`/`maybeFlush`/`scheduleXxx` | `QDateTime::currentMSecsSinceEpoch()` | `nowFn_()` |
| `InboxLedger::markSeen`/`stalePending` | `QDateTime::currentMSecsSinceEpoch()` | `nowFn_()` |
| `OutboundAckStore::updateAcked`/`updateLastSent` | `QDateTime::currentMSecsSinceEpoch()` | `nowFn_()` |
| `SyncWorker::run` 主循环（ackDeadlineMs_ 比较）| `QDateTime::currentMSecsSinceEpoch()` | 经 `SyncConfig` 传入的 `nowFn_` |

测试中通过 `FakeClock::advance(ms)` 快进时间，完全消除 `QThread::sleep` 依赖。
...
| `SqlTraceFixture` | `sqlite3_trace_v2` 包装，捕获 SQL 文本，提供无全表扫断言 | `tst_table_state`/`tst_diff_engine` 等"禁全表扫描"断言 | 临时文件（需 sqlite3*） |
| `ASSERT_ERR_CODE` | 错误码断言宏 | 所有期望失败 + 指定 `E_SYNC_XXX` 的用例 | 不限 |

### 3.9 `SqlTraceFixture`（无全表扫断言，Medium 修复）

**职责**：通过 `SqliteHandle::of(db)` 取 `sqlite3*`，安装 `sqlite3_trace_v2(SQLITE_TRACE_STMT, ...)` 收集执行的 SQL 文本，提供 `assertNoFullTableScan(table)` 断言——检查不出现 `SCAN TABLE <table>` 模式（即无 `EXPLAIN QUERY PLAN` 所见的全表扫）。

**接口草案**：

```cpp
class SqlTraceFixture {
public:
    explicit SqlTraceFixture(QSqlDatabase& db);  // 安装 trace_v2 钩子
    ~SqlTraceFixture();                           // 卸载钩子

    // 断言自安装以来未出现对 table 的全表扫（无 WHERE 索引命中）
    // 实现：检查 capturedSql_ 中未出现 "SCAN TABLE " + table + " " 或行尾
    void assertNoFullTableScan(const QString& table) const;

    // 清空已记录 SQL（用于区分安装前的 seed 操作）
    void clearTrace();

    // 返回所有捕获的 SQL 供手动检查
    QStringList capturedSql() const;
private:
    sqlite3* handle_;
    QStringList capturedSql_;
};
```
...
SqliteFixture fx;
// ... seed 数据 ...
SqlTraceFixture trace(fx.db());
trace.clearTrace();  // 只追踪 applyMutations 之后的 SQL
applyMutations(fx.db(), muts);
trace.assertNoFullTableScan("orders");  // 断言增量路径未全扫 orders
```
...
| `verify_compositePk_rejected` | `02_composite_pk.sql` | `ASSERT_ERR_CODE(verify(db,{t},&rej,&err),"E_SYNC_COMPOSITE_PK_NOT_SUPPORTED")`；MVP 不支持复合 PK（`SchemaEligibility.cpp:82` 明确拒绝）；`rej` 含该表名；`introspect.pkCols.size()>1` → 拒绝 |
...
| `unknownPeer_nodeUnknown_E_SYNC_NODE_UNKNOWN` | 构造一个 `PayloadHeader.routeTag` 指向未在 `configure` 配置内的 peer（如 `"UNKNOWN_NODE"`）；模拟 `SyncWorker::processArtifact` 对此路由调用 `shouldRoute`：期望路由被拒（返回 false）；上层（`SyncWorker`/`processArtifact`）应将此情况上报 `E_SYNC_NODE_UNKNOWN` 到 error 环，**断言**：`errors()` 含 `E_SYNC_NODE_UNKNOWN` 且该制品 ledger 不标 consumed（不应用、不 ACK）。此用例可与 `tst_sync_star_broadcast` 集成场景协作覆盖完整路径。 |
...
| `resolveRecord_compoundPk` | 经 `resolvePk` 解析 `t_compound` 复合 PK（如 `pk=="a1b1"` 或实现约定的复合 PK 编码）→ `rowToPk(row,"t_compound",roConn)` 回算的 pk 与输入一致（往返自洽，验证 `rowToPk` 复合 PK 拼接规范）。 |
...
| `save_stagingConflict_E_SYNC_STAGE_CONFLICT` | **E_SYNC_STAGE_CONFLICT 专用**：构造场景——先 `initialize`，暂存某行 `stageRow("t",pk)`，再从外部连接对同表同 PK 提交一次写（但 `data_version` 相同，即 WAL 内事务同一批），模拟 save 写回时与同期并发裁决冲突（`UpsertExecutor` 返回约束违反）→ `save` 返回 false、`*err` 含 `E_SYNC_STAGE_CONFLICT`（`ASSERT_ERR_CODE`）；staging 不部分提交（事务回滚），gate 已 `releaseAll`（避免死锁）。**此码区分于 `E_SYNC_STAGE_STALE`**：stale 指 data_version 外部写漂移，conflict 指 save 内部 UPSERT 约束冲突。 |
...
- **步骤 / 断言**（四个 data-row，每行一个注入点）：
  ...
  - `after_commit`：COMMIT 后、`markConsumed` 前 kill。断言**事务原子集 4 项全 true**（业务行 + changelog + applied_vector + row_winner 已提交）；**ledger.status==Seen**（consumed 标记在 COMMIT 之后独立进行，尚未执行）。重启后重扫 inbox：`applied_vector` 幂等（seq≤applied_seq → no-op），**不重复消费**；最终 ledger 被标记为 `Consumed`。断言：4 项 true + ledger=Seen（并非 Consumed），不使用 `isAtomic(v)`（5 项统一断言），而是 `QVERIFY(v.businessRowsPresent && v.changelogPresent && v.appliedVectorAdvanced && v.rowWinnerPresent && !v.inboxLedgerConsumed)`。
...
- **步骤 / 断言**：
  ...
  5. **W_SYNC_CONCURRENT_MANUAL_PUSH 专用**：构造两个 Edge（B1、B2）在短窗口内对**同一选择域（同 PK）**同时发起 `syncSelected`，两份制品先后到达 Center：断言 Center 的 `warnings()` 含 `W_SYNC_CONCURRENT_MANUAL_PUSH`，最终**末者胜**（P2 覆盖 P1 via UPSERT DO UPDATE），不自动裁决；两份均幂等处理。
- **关联需求**：上行选择性推送（FK 闭包/剪枝/拓扑分片/直选 DoUpdate-依赖 DoNothing）；`push_progress` done 闸门；M2 DoD 上行闭环；`W_SYNC_CONCURRENT_MANUAL_PUSH`。
...
**SyncWorker 私有函数覆盖矩阵**（高风险细化，2106 行，目标 ≥72%）：

| `SyncWorker` 私有函数 | 近似行范围 | 对应集成套件 |
|---|---|---|
| `run` / `init` 初始化（DDL、store init、consistencyCache init、quarantine drain） | L165–L500 | `tst_sync_two_node_converge`、`tst_sync_eligibility_gate` |
| `processPendingTasks` 任务队列串行 | L506–L516 | `tst_sync_two_node_converge`、`tst_batch_transfer_e2e` |
| `scanInbox` + ledger pending 三时机 + stalePending E_SYNC_GAP | L517–L558 | `tst_sync_strict_sequential`（缺口/pending）、`tst_sync_two_node_converge` |
| `processArtifact` 分派（changeset/selectionpush/baseline/ACK） | L559–L667 | `tst_sync_two_node_converge`、`tst_sync_selective_push`、`tst_sync_baseline_e2e` |
| `processChangesetArtifact`（apply_v2 + 三分支 A） | L668–L751 | `tst_sync_two_node_converge`、`tst_sync_strict_sequential`、`tst_sync_crash_zero_window` |
| `processSelectionPushArtifact`（分支 B + PushChunkAck） | L752–L938 | `tst_sync_selective_push`、`tst_sync_chunk_resume` |
| `processBaselineRequestArtifact` / `processBaselineResponseArtifact` | L939–L1127 | `tst_sync_baseline_e2e` |
| `processAckArtifact`（ChangesetAck + PushChunkAck + ackWaiting_ 清除） | L1128–L1268 | `tst_sync_ack_wait`、`tst_sync_selective_push` |
| `broadcast` + `broadcastTopeer`（readRangeAll + rebase + 路由 + ack window） | L1269–L1411 | `tst_sync_star_broadcast`、`tst_sync_rebase` |
| `isPeerEvicted` / `evaluatePeers`（三维阈值 + evict + outbox 截断） | L1412–L1484 | `tst_sync_dead_peer` |
| `runBaselineFallbackFor`（缺口 → baseline request） | L1485–L1726 | `tst_sync_strict_sequential`（缺口超时→baseline）、`tst_sync_baseline_e2e` |
| `submitCaptureWriteSync`（分支 C 本地写 + originSeq 回滚） | L1750–L1804 | `tst_batch_transfer_e2e`（import 路径） |
| `startAckWait` / `cancelAckWait` / `enqueueDrain` | L1805–L1899 | `tst_sync_ack_wait` |
| `enqueueSelectionPush` + FK 闭包路径 | L1900–L末 | `tst_sync_selective_push`、`tst_sync_chunk_resume` |

CI 在 `SyncWorker.cpp` 单独设门槛：`gcovr --filter src/sync/SyncWorker.cpp --fail-under-line 70`（与整体 80% 并列，任一不过失败）。
...
> 套件命名约定：单元套件取自第 4–11 节的 `tst_<area>`（如 `tst_changeset_applier`）；集成套件取本节 `tst_sync_<scenario>` 命名（如 `tst_sync_strict_sequential`）。§1.4 另以 `tst_apply_crash_window`/`tst_sync_context_single_writer`/`tst_diff_equality_triple`/`tst_selection_strict_contiguity` 标识四个「不变量专用夹具」，本节统一并入对应集成套件（崩溃→`tst_sync_crash_zero_window`、单写者→`tst_sync_context_single_writer`、判等→`tst_sync_comparison_e2e`、严格连续→`tst_sync_strict_sequential`）。
...
| `E_SYNC_UNSUPPORTED_SCHEMA` | eligibility：无 PK 表/视图/影子表/partial 唯一目标表配为同步表，`initialize` attach 前显式校验（G-04/S-02） | 不进入同步模式 | engine 拒绝初始化该表 | `tst_schema_eligibility`、`tst_sync_eligibility_gate` |
| `E_SYNC_COMPOSITE_PK_NOT_SUPPORTED` | eligibility：同步表为复合 PK 表，MVP 不支持（`SchemaEligibility.cpp:82`，`pkCols.size()>1`）；`SchemaEligibility::verify()` 返回 false，`rejected` 含该表名 | 不进入同步模式 | engine 拒绝初始化；需单列 PK 或重新设计表结构 | `tst_schema_eligibility`（用例 `verify_compositePk_rejected`）、`tst_sync_eligibility_gate` |
...
> 错误码覆盖自检：上表 24 个 Error/Fatal（含 `E_SYNC_COMPOSITE_PK_NOT_SUPPORTED`）+ 7 个 Warning = **31 个码，每个码 ≥1 个码级/警告断言**，满足退出准则③。不计入 `E_BUSY`（非 sync 专属，属 ForegroundGate 通用机制，已单独在 `tst_foreground_gate` 覆盖）。
...
| NFR-6 | 单写者 + getter 线程安全 | `tst_write_txn`、`tst_sync_context_single_writer`（并发写串行化/拒绝，G-07）；getter 在 worker 写时并发读无数据竞争（`tst_sync_context_single_writer`，可 TSAN 构建） |
...
| ③ | 全部错误码有码级断言 | §12.1 矩阵 31 码（24 Error/Fatal + 7 Warning）逐一映射到 `ASSERT_ERR_CODE`/warning 断言；CI 以 grep 校验每个码常量在 `tst_*` 中至少出现一次（辅助），主要靠 ctest XML 解析确认对应 slot 执行成功（`tests/sync_error_matrix.json` 表驱动 manifest，每个码对应 suite::slot） |
...
| 步 | 阶段 | 命令 / 动作 | 失败动作 |
| --- | --- | --- | --- |
| 1 | 构建 | CMake：`cmake -S . -B build-cov -DBUILD_TESTING=ON -DDBRIDGE_COVERAGE=ON -DCMAKE_BUILD_TYPE=Debug && cmake --build build-cov -j`；qmake 路径：先校验 `libdbridge.pro` 与 CMake SOURCES 一致（§2.2.2 前置任务），再影子构建 `libdbridge.a`/`libQXlsx.a` | 编译/链接失败即红 |
...
| **M1a** 基础/DDL/写线程/key/eligibility | `Errors.h` 全量码、`SqliteHandle`/`WriteTxn`/`ForegroundGate`、`SyncContext` key 加固 | `tst_write_txn`、`tst_sync_context_single_writer`、`tst_foreground_gate`、`tst_sync_context`、`tst_schema_eligibility`、`tst_sync_eligibility_gate`、`tst_sync_config_builder`、`tst_sync_selection_builder` | 写串行入队；路径别名同库单 context（G-07）；无 PK/视图→`E_SYNC_UNSUPPORTED_SCHEMA`（G-04）；崩溃夹具可注入点杀进程 |
