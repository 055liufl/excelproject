# Capability: time-format

## Purpose

Defines how the system handles temporal data (dates, datetimes, times) across the Excel and SQLite boundaries. A profile may declare named format slots (`dateFormat`, `datetimeFormat`, `timeFormat`) at the profile root and/or per column, controlling how values are parsed on import and serialized on export.
...
When the Excel cell read by `ExcelReader` is already a structured temporal value (`QVariant::type()` equals `QVariant::Date`, `QVariant::DateTime`, or `QVariant::Time`), the system SHALL NOT apply `excel.format` parsing. Instead, the structured value SHALL proceed directly to the db-side serialize step (import) or directly to the equivalent step in the export path. `excel.fallback` SHALL also be bypassed in this case.

#### Scenario: Native QDate cell goes straight to db-side serialize
- **WHEN** Excel returns a `QVariant(QDate(2025,3,14))` for a column governed by `dateFormat: {excel:{type:"string",format:"yyyy-MM-dd"}, db:{type:"string",format:"yyyy/M/d"}}`
- **THEN** the value bound to SQL SHALL be `"2025/3/14"` (no `fromString` invoked)

#### Scenario: Native QDateTime cell goes straight to db-side serialize
- **WHEN** Excel returns a `QVariant(QDateTime)` for a column governed by `datetimeFormat`
- **THEN** the structured value SHALL be serialized via the effective `db` side spec and bound to SQL

#### Scenario: Native QTime cell goes straight to db-side serialize
- **WHEN** Excel returns a `QVariant(QTime(14,30))` for a column governed by `timeFormat: {db:{type:"string",format:"HH:mm:ss"}}`
...
1. **Both declared on the same column** → the new `dateFormat` SHALL take precedence. The `date:fmt` validator SHALL still run but SHALL behave as a pass-through for values that are already `QDate` / `QDateTime` (its existing fast-path at `Validators.cpp:133`); the `excel.format` of `dateFormat` SHALL be the actual parser. Profile loading SHALL emit an info-level diagnostic noting that `dateFormat` overrides `date:fmt`.
2. **Only `validators: ["date:fmt"]`, no `dateFormat`** → legacy behavior: parsing uses `fmt`; the DB-side format is the implicit ISO `"yyyy-MM-dd"`; no export-side V parsing occurs (current behavior); no `excel.fallback` available.
3. **Only `dateFormat`, no `date:fmt`** → the system SHALL behave **as if** an implicit `date:dateFormat.excel.format` validator were attached for purposes of type assertion in downstream code paths that key off validator presence; this implicit validator SHALL NOT introduce additional failure modes beyond those already specified by this capability.
...
- **WHEN** a column declares `dateFormat: {excel:{type:"string",format:"yyyy/M/d"}, db:{type:"string",format:"yyyy-MM-dd"}}` AND `validators: ["date:yyyy-MM-dd"]`
...
- **WHEN** a column declares `dateFormat: {excel:{type:"string",format:"yyyy-MM-dd"}, db:{type:"string",format:"yyyy-MM-dd"}}` and no `date:` validator
...
- `E_TIME_PARSE` — import-direction parse failure (Excel string did not match `excel.format` or any fallback). The row SHALL be skipped for the declaring route; the existing fk-injection error-cascade rule SHALL apply for child routes.
- `E_TIME_PARSE_DB` — export-direction parse failure (DB value did not match `db.format`, or failed epochSec conversion). The single offending cell SHALL be written as NULL and the rest of the row SHALL continue.

Both codes SHALL include the column name, the offending raw value, and the format string (or type) that was being applied.
