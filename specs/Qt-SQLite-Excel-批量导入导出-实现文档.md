...
| 3. 未来未知单表 + 未来复杂表用 JSON Profile | `SchemaIntrospector` + `AutoProfileBuilder` + `ProfileLoader` | §6.9 自省 + 自动 Profile 生成 | §11.3 `auto_profile_single`、`auto_profile_no_unique_key` |
| 4. 单类行（A 行）拆入多表 m 集合 | `Mapper`（多 route）+ `TopoSorter` + `FkInjector` | §6.6 业务键 FK 注入 + §6.3 拓扑排序 | §11.3 `multi_table_split_and_export` |
| 5. 同 Sheet 混编 A/B/C 分发到 m/n/o 多表集合 | `Router`（鉴别列）+ `Mapper`（按 class 选 routes） | §6.7 Discriminator 匹配 + §6.8 expandRows | §11.3 `mixed_abc_roundtrip` |
...
|---|---|
| C++ 标准 | C++17 |
| Qt | 5.12.12（`QtCore` / `QtSql`；QXlsx 私有依赖 `QtGui`，不引入 `QtWidgets`） |
| 第三方 | QXlsx（vendored 进 `3rdparty/QXlsx/`） |
| 数据库 | 仅 SQLite 3.24+（支持 `ON CONFLICT ... DO UPDATE`；运行期必须检查 `sqlite_version()`） |
| 构建 | CMake ≥ 3.16 |
...
- SQLite 直接使用 `QSQLITE` 驱动（Qt 自带，无需链接 sqlite3 源码）。由于 Qt 5.12.12 的 `QSQLITE` 可能使用 Qt 自带或系统 SQLite，`DataBridge::open` 必须执行 `SELECT sqlite_version()`，低于 3.24.0 时返回 `E_OPEN_DB`。
...
generate_export_header(dbridge
  BASE_NAME DBRIDGE
  EXPORT_FILE_NAME ${CMAKE_CURRENT_BINARY_DIR}/include/dbridge/Export.h)
...
target_link_libraries(dbridge PUBLIC Qt5::Core Qt5::Sql PRIVATE QXlsx Qt5::Gui)
...
输出形态：静态库或动态库由 `BUILD_SHARED_LIBS` 决定，宿主侧链接 `dbridge`。Windows 动态库构建必须使用生成的 `dbridge/Export.h`，并给所有 public class 标注导出宏。
...
struct ExportSpec {
    QStringList orderBy;
    QString     explicitSql;     // 仅 singleTable / multiTable 使用；mixed 必须为空
...
`ProfileLoader` 读入 JSON 后立即归一化为上面的内部结构：

- `mode=singleTable` 的顶层 `table` / `conflict` / `columns` 会转换成一个无 `parent` 的根 `RouteSpec`，放入 `ProfileSpec.routes[0]`。
- `mode=multiTable` 的 `routes[]` 逐项转换为 `RouteSpec`；`fkInject.from/to` 形如 `orders.order_no`，解析成 `fromTable/fromColumn/toTable/toColumn`。
- `mode=mixed` 的每个 `classes[].routes[]` 独立转换；顶层 `discriminator.source` 写入 `discriminatorSource`，`match.equals` 写入 `ClassSpec.matchEquals`。
- `export.sql` 归一化为 `ExportSpec.explicitSql`，`export.classColumn` 归一化为 `ExportSpec.classColumn`。
- `mixed` 模式不接受全局 `export.sql`；如需自定义导出 SQL，必须改为单表/多表模式或进入长期演进文档。
...
- `singleTable` 必须有顶层 `table`；`multiTable` 必须有非空 `routes`；`mixed` 必须有非空 `classes`，且每个 class 必须有非空 `routes`。
- `columns` 可以为空，但只有非 root route 且所有 conflict 列都能由 `fkInject` 注入时才可落地；root route 的业务键必须显式映射。否则 ProfileValidator 返回 `E_PROFILE_NO_CONFLICT_KEY`。
- `validators` 数组按顺序保留为字符串 token，不在 Loader 内执行行级语义；但 token 格式必须在加载阶段编译校验，未知 token 或格式错误立即返回 `E_PROFILE_PARSE`，避免导入运行到一半才发现 Profile 写错。
- `export.sql` 仅允许在 SingleTable / MultiTable Profile 中出现；Mixed 模式不接受 `exportSpec.explicitSql`，统一走自动 LEFT JOIN，避免一条全局 SQL 被重复执行多次。

### 5.2 SchemaIntrospector

文件：`src/schema/SchemaIntrospector.{h,cpp}`。

职责：对一个已经 `QSqlDatabase::open()` 的连接，遍历所有表填 `SchemaCatalog`。在 `DataBridge::open` 成功后立即扫描一次；`generateAutoProfileJson()` 与每次 `loadProfile*()` / `importExcel()` / `exportExcel()` 进入 Profile 校验前刷新一次 catalog，保证运行期新增表可被识别。
...
2. 对每个 RouteSpec：
   ...
   - 每个 conflict 列必须能从 `columns` 映射得到，或作为子 route 的 `fkInject.toColumn` 得到；root route 缺少任一 conflict 值来源 → `E_PROFILE_NO_CONFLICT_KEY`。
...
5. 标识符校验：所有 table / column / alias / orderBy 字段只接受 SQLite 简单标识符格式 `[A-Za-z_][A-Za-z0-9_]*`；`orderBy` 可为 `table.column`。不接受引号、空格、函数调用或表达式。SingleTable / MultiTable 的 `export.sql` 是唯一允许业务方直接提供 SQL 的位置。
...
每个 validator 是一个 `std::function<bool(const QVariant& in, QVariant* out, QString* errCode, QString* msg)>`。`ValidatorChain` 把一个 token 数组在 Profile 加载完成后**一次编译成函数列表**，后续按行执行不再 parse 字符串。
...
- 只在 Profile 加载 / ValidatorChain 编译阶段解析 token；行级执行阶段不得再解析 token 字符串。
- `regex` 必须用 `QRegularExpression::anchoredPattern(pattern)` 或等效方式做全匹配，不使用包含匹配。
- 多个类型规范化 token 同时出现时，以最后一个成功产生的规范化值为准；不允许两个互斥类型 token（例如 `int` 与 `date:...`）同时配置，编译阶段返回 `E_PROFILE_PARSE`。
...
若某个 `conflict.columns` 不在 `payload.dbColumns` 中：

- 该列可以由后续 `FkInjector` 注入时，Mapper 先保留空位，FK 注入后再填充 `conflictVals`。
- 该列既不在 `columns` 中、也不是 `fkInject.toColumn` 时，ProfileValidator 必须在导入前报 `E_PROFILE_NO_CONFLICT_KEY`；否则 Upsert 无法构造完整 conflict key。
...
- SingleTable / MultiTable：执行单条 SQL（`export.sql` 归一化后的 `exportSpec.explicitSql` 优先；否则自动生成）→ 按返回列顺序写表头与数据。
...
SingleTable / MultiTable 的 `export.sql` 存在时直接执行该 SQL，列别名（AS）必须与 Profile 中 ColumnSpec.source 一致，否则导出表头不可知。
...
实现：`QHash<QString, QHash<QString, SeenPayload>>`，外层 key 是 `route.table`（或 `classId:table`），内层 key 是 conflict 值的 join 串（`|` 分隔，前置长度避免歧义），value 保存首次出现的 Excel 行号与规范化 payload。
...
for payload in row.payloads:
    ...
    if seen[payload.routeKey].contains(key):
        first = seen[payload.routeKey][key]
        if route_has_children(payload.routeKey) && payload.binds == first.payload.binds:
            skip duplicate payload   // 完全相同的父行重复出现时允许去重
        else:
            errors.add(row=excelRow, col=conflictColsJoin,
                       raw=key, code=E_VALIDATE_DUPLICATE,
                       msg=fmt("conflict key 与第 %1 行冲突").arg(first.excelRow))
    else:
        seen[payload.routeKey][key] = {excelRow, payload}
```

`encodeConflict` 把值拼成 `<len>|<v1>|<len>|<v2>|...` 形式以避免 `"ab|c"` 与 `"ab","c"` 撞键。

去重只用于多表一对多的父 route：同一张订单的父字段在多条明细行中重复出现时，父 payload 保留一份即可。没有子 route 的普通单表重复行，或同一 conflict key 但非 key 字段不一致的行，仍然返回 `E_VALIDATE_DUPLICATE`。

### 6.5 外键存在性预校验

MVP 要求导入前完成外键存在性校验，避免等到写阶段才由 SQLite 报错。实现只覆盖最小可落地场景：

1. 对每个带 `fkInject` 的子 payload，先判断父 payload 是否在同一行或同一批中可生成业务键；能生成则通过父子同批校验。
2. 对 Profile 中映射到真实 SQLite FK 的列，如果引用父行不在本批 payload 中，则在 Phase C 结束后、开启写事务前，按 `(refTable, refColumn, value)` 去重后执行 `SELECT 1 FROM <refTable> WHERE <refColumn> = ? LIMIT 1`。
3. 查不到时写 `E_VALIDATE_FK`，错误定位到子行、子列与原始值；只要有任意 FK 错误，就不进入 Phase D。

边界：

- MVP 只支持单列业务键 FK 预校验；复合 FK 若无法由 `fkInject` 明确表达，要求业务方用 `export.sql` / Profile 调整规避，或进入长期演进。
- 预校验 SQL 的表名/列名来自已通过 ProfileValidator 的标识符，值一律用 bind 参数。
...
DataBridge::importExcel
  └── ImportService::run(profile, xlsxPath, opts)
       ...
       Phase C: 全量行映射 + 行级校验
         ...
         6. ForeignKeyPreflight.check(contexts)       // 外部父行不存在 -> E_VALIDATE_FK
         7. if errors.notEmpty(): return ok=false
       Phase D: 落库（单事务）
         8. if !db.transaction(): return E_DB_UPSERT
         9. for ctx in contexts:
              ...
              writtenRows++
        10. if errors.empty():
                if !db.commit(): db.rollback(); return E_DB_UPSERT
            else:
                db.rollback()
       ...
       2. branch by mode:
          ...
          b. mixed:
             ...
             for cls in profile.classes:
                sql = autoJoin(cls.routes, exportSpec)
                rs  = exec(sql)
                while rs.next():
...
DataBridge::generateAutoProfileJson(table)
  1. SchemaIntrospector.load(db, &catalog) 刷新表结构
  2. catalog.hasTable(table) ? else E_PROFILE_TABLE_NOT_FOUND
  3. AutoProfileBuilder.build(catalog.table(table)) -> ProfileSpec
  4. ProfileSpec -> JSON string (固定字段顺序)
  5. 不写入内部 profile 缓存（由宿主决定是否再 loadProfileFromString）
```
...
- `db.transaction()`、`db.commit()`、`db.rollback()` 的返回值都必须检查；任一失败都视为写入失败并保证 `writtenRows = 0`。
- 在 `DataBridge::open` 中设置：

```sql
SELECT sqlite_version();                    -- 必须 >= 3.24.0
...
- `DataBridge` 的所有公开方法必须在同一线程内调用。若宿主需要后台执行，应在目标工作线程中创建 `DataBridge`、调用 `open()`、`loadProfile*()`、`importExcel()` / `exportExcel()` 并在同线程 `close()`；不允许 UI 线程 `open()` 后再切换到别的线程复用同一实例。
...
| 2.5 | 外键存在性预校验（§6.5） | `src/validation/ForeignKeyPreflight.*`（可放在 service/mapping 同目录） | 单元测试：DB 已有父行通过；缺父行返回 `E_VALIDATE_FK` |
| 2.6 | 行级校验失败时严格不开事务 | `src/service/ImportService.*` | 集成测试：故意造一行错误，DB 中应无任何写入 |
...
| `validate_fk_missing` | 子表引用的父业务键既不在本批也不在 DB → `E_VALIDATE_FK`，DB 零写入 |
| `multitable_duplicate_parent_allowed` | 同一订单多行明细重复出现相同父 payload → 父 payload 去重，子 payload 按完整 conflict key 校验 |
...
- [ ] 多表一对多导入时，相同父业务键且父字段一致的重复父 payload 被去重，不阻断多个子行写入。
