# Capability: export-column-order

## Purpose

Allows a profile's `exportSpec` to declare an explicit `columnOrder` array, overriding the default (historical) column ordering in exported Excel sheets. Listed headers appear first in declared order; unlisted headers are appended in their natural order as a stable suffix.
