...
Defines how child routes receive foreign-key column values from parent routes via the `fkInject` declaration. Covers multi-column and multi-parent injection, conflict-value alignment, preflight suppression for lookup-derived groups, NULL handling, error cascade semantics, and profile validation rules for `fkInject`.
