...
Defines how routes declare and execute lookups against parameter tables in the same SQLite database to enrich import payloads with reference data. Covers declaration syntax, batch prefetch with identity merging, Mixed-mode scoping, coercion, cardinality semantics, strict equality, NULL propagation, validator bypass, conflict-column participation, route-local visibility, dbColumn uniqueness, schema validation, and no-cascading constraints.
